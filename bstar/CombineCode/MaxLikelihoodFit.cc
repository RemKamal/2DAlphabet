  ...
  if (currentToy_ < 1){
    ...
    if (!justFit_ && nuis && globalObs ) {
      ...
      if (saveNormalizations_) {
          ...
          get2DNormalizations(mc_s->GetPdf(), *mc_s->GetObservables(), *norms, sampler, currentToy_<1 ? fitOut.get() : 0, "_prefit",data);
          delete norms;
      }
  ...
  if (res_b) { 
     ...
     fitStatus_ = res_b->status();
      ...
      if (saveNormalizations_) {
          ...
          get2DNormalizations(mc_s->GetPdf(), *mc_s->GetObservables(), *norms, sampler, currentToy_<1 ? fitOut.get() : 0, "_fit_b",data);
  ...
  if (res_s) { 
     ...
     nll_nll0_ =  nll_sb_ -  nll_bonly_;
      ...
      if (saveNormalizations_) {
          ...
          get2DNormalizations(mc_s->GetPdf(), *mc_s->GetObservables(), *norms, sampler, currentToy_<1 ? fitOut.get() : 0, "_fit_s",data);
          setNormsFitResultTrees(norms,processNormalizations_);
      delete norms;
      }
...
void MaxLikelihoodFit::get2DNormalizations(RooAbsPdf *pdf, const RooArgSet &obs, RooArgSet &out, NuisanceSampler & sampler, TDirectory *fOut, const std::string &postfix,RooAbsData &data) {
    // fill in a map
    ...
    typedef std::map<std::string,TGraph2DErrors*>::const_iterator IG;
    ...
    std::map<std::string,TGraph2DErrors*> datByCh;
    if (shapeDir) {
        for (IT it = snm.begin(), ed = snm.end(); it != ed; ++it) {
            ...
            if (sub == 0) {
                ...
                RooRealVar *y = (RooRealVar*)it->second.obs.at(1);
                std::cout << "Printing y var: ";
                y->Print();
                RooDataHist *dataCut = (RooDataHist*)data.reduce(RooFit::Cut(TString("CMS_channel==CMS_channel::"+it->second.channel)));
                int xNbins = x->getBinning().numBins();
                int yNbins = y->getBinning().numBins();
                char xVarName = *x->GetName();
                char yVarName = *y->GetName();
                std::string varString = std::string("")+xVarName+","+yVarName +"";
                std::cout << varString << ": [" << std::to_string(xNbins) << ", " << std::to_string(yNbins) << "]" << std::endl;
                TH1* dH = dataCut->createHistogram(varString.c_str(),xNbins,yNbins);
                //TH1* dH = dataCut->createHistogram("("+xVarName+","+yVarName+")",xNbins,yNbins);
                dH->SetName("data");
                TGraph2DErrors * dGraph = utils::makeData2DGraph(dH,/*asDensity*/true);
                datByCh[it->second.channel] = (TGraph2DErrors *) dGraph->Clone();
    ...
    std::cout << "datByCh size " << datByCh.size() << std::endl;
    ...
    if (datOverallHist->GetDimension() == 2) {
      TGraph2DErrors * datOverall = utils::makeData2DGraph(datOverallHist);
    } else {
      TGraphAssymErrors * datOverall = utils::makeDataGraph(datOverallHist);
    }
    ...
    if (shapeDir) {
        for (IT it = snm.begin(), ed = snm.end(); it != ed; ++it) {
            ...
            if (sub == 0) {
                ...
                datByCh[it->second.channel]->SetNameTitle("data", (it->second.process+" in "+it->second.channel).c_str());
                // std::map<std::string,TGraphAsymmErrors*> datByCh = dat1DByCh;
