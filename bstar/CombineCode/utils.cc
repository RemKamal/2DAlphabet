...
TGraph2DErrors * utils::makeData2DGraph(TH1 * dataHist, bool asDensity)
{
  // Properly normalise 
  TGraph2DErrors * dataGraph = new TGraph2DErrors();

  dataHist->Sumw2(false);
  int ipoint = 0;
  // dataHist->SetBinErrorOption(TH1::kPoisson);
  for (int bx=1;bx <= dataHist->GetNbinsX();bx++){
    for (int by=1;by <= dataHist->GetNbinsY();by++){
      double zv = dataHist->GetBinContent(bx,by);
      double bwx = dataHist->GetXaxis()->GetBinWidth(bx);
      double bwy = dataHist->GetYaxis()->GetBinWidth(by);
  
      double err = dataHist->GetBinError(bx,by);

      if (asDensity) {
        zv/=(bwx*bwy);
        err/=(bwx*bwy);
      }
      dataGraph->SetPoint(ipoint,dataHist->GetXaxis()->GetBinCenter(bx),dataHist->GetYaxis()->GetBinCenter(by),zv);
      dataGraph->SetPointError(ipoint,bwx/2,bwy/2,err);
      ipoint++;
    }
  }
