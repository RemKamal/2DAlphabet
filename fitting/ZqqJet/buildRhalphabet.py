...
class dazsleRhalphabetBuilder: 
	...
	def makeWorkspace(self,iOutput,iDatas,iFuncs,iVars,iCat="cat0",iShift=True):
		...
		for pFunc in iFuncs:
			...
			if iShift and ("wqq" in process or "zqq" in process):
				...
				# NOTE - It seems like we've done all of this in other parts of the code...

				if pFunc.GetName() == "zqq300_pass_cat5": # bork the last 2 bins!
					tmph_mass_matched.SetBinContent(59,0.);
					tmph_mass_matched.SetBinContent(59,0.);		# NOTE - this block should be removed as it's an analysis specific fix
				...
				# NOTE - I'm just going to trust this works
				...
				# NOTE - Again these are all specific to this analysis' signal and will probably be done away with
				...
				for h in hout:
					# again,get rid of very low or high mass bins according to pT cat
					# NOTE - Still too specific for b* most likely
					for i0 in range(1,self._mass_nbins+1):
						if ((i0 > 31 or i0 < 0) and int(ipt) == 1) or ((i0 > 38 or i0 < 4) and int(ipt) == 2) or ((i0 > 46 or i0 < 5) and int(ipt) == 3) or ((i0 > 56 or i0 < 7) and int(ipt) == 4) or ( i0 < 7 and int(ipt) == 5):
		...
		# NOTE - Up to this point in this function, there's been nothing done but 'shift it if you need to, otherwise just save it to the new workspace'

		# do the signal interpolation
		# NOTE - Most likely will be removed
		print "---------------------------------------------------------------"
		...
		for m in interpolatedMasses:
			...
			# NOTE - another analysis specific fix that gets rid of signal at the high/low ends
			...
			for h in hout:
				print h.GetName()
				# NOTE - again with the 'set bin to zero'
				for i0 in range(1,self._mass_nbins+1):
				...
				# Replace 'scale' with 'scalept' in the histo name, convert to Roo, import to workspace
				# NOTE - Doesn't this make duplicates (will have 'scale' and 'scalept'?)
				if h.GetName().find("scale") > -1:
					pName=h.GetName().replace("scale","scalept")
					tmprdh = RooDataHist(pName,pName,r.RooArgList(self._lMSD),h)
					getattr(lW,'import')(tmprdh, r.RooFit.RecycleConflictNodes())
	...
	def buildPolynomialArray(self, iVars,iNVar0,iNVar1,iLabel0,iLabel1,iXMin0,iXMax0):
		...
		# NOTE - mlfit_param.root is the initial 'guess' values for the fit and is generated
		...
		for i0 in range(iNVar0+1):
			for i1 in range(iNVar1+1):
				...
				# NOTE - The above seems highly specific and will probably need to change for b*

				# Guess 0 for p0r0 and for all else, grab the value from lFit
				# NOTE - not sure what the difference between pCent != 0 and pVar is
	...
	def rooTheHistFunc(self,iH,iLabel="w",iBin="_cat0"):
		...
		# Build pdfs - NOTE - Need lShift here to float the SHAPE. But what would happen if you constructed without it?
